...
export default {
  name: "label-data-summary-keepAlive",
  data() {
    return {
      noneStatus: false,
      datasetList: [],
      columns1: [
        ...
        },
        {
          ...
          render: (h, parmas) => {
            let a = 0;
            let b = 0;
            if (parmas.row.picNum != 0)
              a = (parmas.row.picCheckNum / parmas.row.picNum).toFixed(2);
            if (parmas.row.picCheckNum != 0)
              a = (parmas.row.picSuccessNum / parmas.row.picCheckNum).toFixed(
                2
              );
            let str = `${a} / ${b} `;
            return h("div", str);
        ...
        },
        {
          ...
          render: (h, parmas) => {
            let a = 0;
            let b = 0;
            if (parmas.row.tagNum != 0)
              a = (parmas.row.tagCheckNum / parmas.row.tagNum).toFixed(2);
            if (parmas.row.tagCheckNum != 0)
              a = (parmas.row.tagSuccessNum / parmas.row.tagCheckNum).toFixed(
                2
              );
            let str = `${a} / ${b} `;
            return h("div", str);
